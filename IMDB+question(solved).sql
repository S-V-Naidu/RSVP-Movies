--This SQL operations were done usng an Online SQL compiler (SQL Online IDE) and later copied in a text document, please excuse if any formatting has gone wrong
USE imdb;
...
-- Here we are counting the total rows in each table sequentially.

SELECT COUNT(*) AS 'Rows in Director Mapping Table'
FROM director_mapping;

SELECT COUNT(*) AS 'Rows in Genre Table'
FROM genre;

SELECT COUNT(*) AS 'Rows in Movie Table'
FROM movie;

SELECT COUNT(*) AS 'Rows in names Table'
FROM names;

SELECT COUNT(*) AS 'Rows in Ratings Table'
FROM ratings;

SELECT COUNT(*) AS 'Rows in Role Mapping Table'
FROM role_mapping;


-- Q2. Which columns in the movie table have null values?
-- Since we already know the columns in each table, here we count the number of NULL values present in each column of the Movies Table
...
FROM movie;
--Columns: 'country', 'worldwide_gross_income', 'languages', 'production_company' have NULL values
...
ON g.movie_id = m.id
WHERE year = 2019 -- since it is mentioned in the question to check for last year
GROUP BY genre
...
(
           SELECT     d.name_id,
                      ...
                      r.total_votes,
                      m.date_published,
                      LEAD(m.date_published,1) OVER(PARTITION BY d.name_id ORDER BY date_published,movie_id ) AS next_date_published
           FROM director_mapping AS d
           INNER JOIN names AS n
           ON         n.id = d.name_id
           INNER JOIN movie AS m
